...
const accountRegex = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
let testText;
account.forEach(accountText => {
  accountText.addEventListener('change', () => {
    clearTimeout(testText);
    testText = setTimeout(() => {
      accountText.style.borderBottom = accountRegex.test(accountText.value) ? '2px solid #FFBEBA' : '2px solid red';
    }, 200);
  });
});
...
password.forEach(passwordText => {
  passwordText.addEventListener('change', () => {
    clearTimeout(testText);
    testText = setTimeout(() => {
      passwordText.style.borderBottom = passwordRegex.test(passwordText.value) ? '2px solid #FFBEBA' : '2px solid red';
    }, 200);
  });
});
...
passwordCheck.addEventListener('change', () => {
  clearTimeout(testText);
  testText = setTimeout(() => {
    passwordCheck.style.borderBottom = passwordCheck.value === password[0].value && passwordRegex.test(passwordCheck.value) ? '2px solid #FFBEBA' : '2px solid red';
  }, 200);
});
