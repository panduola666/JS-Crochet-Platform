...
searchBtn.addEventListener('click',(e) => {
    ...
    if(searchChange.textContent === '作品名　>'){
        ...
        if(searchContent.value ===''){
            ...
            location.href = `/articleList?search=${ localStorage.getItem('search') }`;
        };
    };
});
//header Icon
if( localStorage.getItem('userId') ){
    ...
    userMore.addEventListener('click', (e) => {
        if(e.target.classList.contains('userIcon')){
            axios(`${baseUrl}/600/users/${ localStorage.getItem('userId') }`, headers)
            .then(res => window.location.href=`/user/${res.data.id}`)
            .catch(() => {
                localStorage.clear();
...
shoppingCarIcon.addEventListener('click', () => {
    axios(`${baseUrl}/600/users/${ localStorage.getItem('userId') }`, headers)
    .then( res => window.location.href=`/shoppingCar/${res.data.id}` )
    .catch(() => {
        localStorage.clear();
        Swal.fire({
            icon: 'error',
