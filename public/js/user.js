...
baseDBList.forEach(li => {
    li.addEventListener('click',(e) => {
        if( li.textContent.includes('我的文章') ){
            document.location.href = `/myArticle/${ userDb.id }`;
        }else if( li.textContent.includes('我的留言') ){
            document.location.href = `/myMessage/${ userDb.id }`;
        }else if( li.textContent.includes('我的收藏') ){
            document.location.href = `/mySave/${ userDb.id }`;
        }else if( li.textContent.includes('材料包管理') ){
            document.location.href = `/userSell/${ userDb.id }`;
        };
    });
});
...
passwordChange.addEventListener('click',() => {
    ...
    if(passwordChange.textContent === '編輯'){
        ...
        pwInput.forEach(input => {
            ...
            //密碼驗證
            pwInput.forEach(input => {
                input.addEventListener('keyup',() => {input.style.borderBottomColor = password_regex.test(input.value) ? '#1F0707' : 'red'});
            });
