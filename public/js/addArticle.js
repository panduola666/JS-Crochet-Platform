...
const tagType_input = tagType.querySelectorAll('input');
...
.then(res=>{
    ...
    if( localStorage.getItem('works') ){
        ...
        .then(res=>{
            ...
            tagType_input.forEach(input =>{     input.value === res.data.tag ? input.click() : '' });
            if( res.data.isSell ){
                //有販售內容
                needSell.click();
...
function chooseConfig(){
    ...
    articleType.addEventListener('click',(e)=>{
        ...
        tagType_input.forEach(input => input.checked = false );
        //標籤篩選
        if(e.target.value === 'works'){
            tagType_input.forEach(input => input.disabled = false );
            for (let i = tagType_input.length-1; i >= tagType_input.length-3; i--) {
                tagType_input[i].disabled = true;
            };
    ...
    isSell.addEventListener('click',(e)=>{
        ...
        if(e.target.value === '是,我要販售材料包:'){
            const sellBy_input = document.querySelectorAll('.sellBy input[type=radio]');
            sellBy_input.forEach(input => input.checked = false );
...
function renderOption(){
    ...
    goodsData.forEach(item=>{
        if(item.type !== '材料包') materialStr.push(`<option value="${ item.title }">${ item.title }</option>`);
    });
