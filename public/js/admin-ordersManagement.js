...
searchOrderId.addEventListener('click',()=>{
    ...
    searchOrderTable.innerHTML = searchOrderMsgStr;
        ...
        if(item.state === '待處理'){
            pendingStr.push(
                `<tr>
                    ...
                    <td class="buyerInfo" data-id="${item.id}">
...
function orderStateChange(){
    ...
    orderState.forEach(state=>{
        state.addEventListener('change',()=>{
            ...
            axios.patch(`${baseUrl}/orders/${state.dataset.id}`,{
                state : state.value,
